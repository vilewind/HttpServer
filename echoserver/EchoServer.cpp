...
Server::Server()
    : m_soc(createNoblockSocket()),
      m_addr("127.0.0.1", 8888),
      m_epfd(ERRIF("epoll create", 0, epoll_create1, EPOLL_CLOEXEC))
{
    m_soc.bind(m_addr);
    m_soc.listen();
...
void Server::epollAdd(epoll_event& ee) {
    int fd = ee.data.fd;
...
void Server::loop() {
    while(1) {
        int num = ERRIF(__func__, 0, epoll_wait, m_epfd, &*m_eventLs.begin(), m_eventLs.capacity(), 0);
        if (num == 0) {
