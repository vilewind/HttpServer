...
HttpParse::LINE_STATUS HttpParse::parse_line()
{
    ...
    {
        int idxr = m_buf.find(CR, m_lineStartIdx);
        int idxn = m_buf.find(LF, m_lineStartIdx);
    /* "\n"在"\r"前或有"\n"没有"\r"说明格式错误；否则，行不完整*/   
        if (idxr > idxn || (idxr == m_buf.npos && idxn != m_buf.npos))
        {
            return LINE_BAD;
        }
        return LINE_OPEN;
...
HttpParse::HTTP_CODE HttpParse::parse_process()
{
            ...
            || ( ( line_status = parse_line() ) == LINE_OK ) )
    {
        ...
        case CHECK_STATE_HEADER :
            ret = parse_headers();
            if (ret == BAD_REQUEST || ret == GET_REQUEST)
                return ret;
            break;
